...
apr_status_t create_window(WNDCLASSEX& wc, HWND& hwnd) {
...
apr_status_t setup_imgui_context(HWND hwnd) {
    ...
    ImGui_ImplDX12_Init(g_pd3dDevice, NUM_FRAMES_IN_FLIGHT, DXGI_FORMAT_R8G8B8A8_UNORM, g_pd3dSrvDescHeap, g_pd3dSrvDescHeap->GetCPUDescriptorHandleForHeapStart(),
    ...
    io.Fonts->AddFontFromFileTTF(RESOURCE_DIR "\\UbuntuMono-R.ttf", 16.0f);
    // ImFont* font = io.Fonts->AddFontFromFileTTF("c:\\Windows\\Fonts\\ArialUni.ttf", 18.0f, NULL, io.Fonts->GetGlyphRangesJapanese());
    // IM_ASSERT(font != NULL);

    return 0;
}
typedef struct our_state_s {
...
void main_loop(uv_idle_t *handle) {
    our_state_s* state = static_cast<our_state_s*>(handle->data);
    if (state->msg.message == WM_QUIT) {
        uv_idle_stop(handle);
        return;
    }
    ...
    if (::PeekMessage(&state->msg, NULL, 0U, 0U, PM_REMOVE)) {
    ...
    g_pd3dCommandList->ClearRenderTargetView(g_mainRenderTargetDescriptor[backBufferIdx], (float*)&state->clear_color, 0, NULL);
...
#pragma region clear window and d3d resource
